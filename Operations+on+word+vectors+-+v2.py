...
def complete_analogy(word_a, word_b, word_c, word_to_vec_map):
    ...
    for w in words:        
        ...
        cosine_sim = cosine_similarity(e_b-e_a, word_to_vec_map.get(w)-e_c)
...
def neutralize(word, g, word_to_vec_map):
    ...
    e_biascomponent = np.dot(e,g)/np.power((np.linalg.norm(g)),2)*g
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    w1 = pair[0]
    w2 = pair[1]
    e_w1 = word_to_vec_map.get(w1)
    e_w2 = word_to_vec_map.get(w2)
    
    # Step 2: Compute the mean of e_w1 and e_w2 (≈ 1 line)
    mu = (e_w1+e_w2)/2

    # Step 3: Compute the projections of mu over the bias axis and the orthogonal axis (≈ 2 lines)
    mu_B = np.dot(mu,bias_axis)/(np.power(np.linalg.norm(bias_axis),2))*bias_axis
    mu_orth = mu-mu_B

    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (≈2 lines)
    e_w1B = np.dot(e_w1,bias_axis)/(np.power(np.linalg.norm(bias_axis),2))*bias_axis
    e_w2B = np.dot(e_w2,bias_axis)/(np.power(np.linalg.norm(bias_axis),2))*bias_axis
        
    # Step 5: Adjust the Bias part of e_w1B and e_w2B using the formulas (9) and (10) given above (≈2 lines)
    corrected_e_w1B = np.sqrt(np.abs(1-np.power(np.linalg.norm(mu_orth),2)))*((e_w1B-mu_B)/np.abs((e_w1-mu_orth)-mu_B))
    corrected_e_w2B = np.sqrt(np.abs(1-np.power(np.linalg.norm(mu_orth),2)))*((e_w2B-mu_B)/np.abs((e_w2-mu_orth)-mu_B))
